...
#### Model Options (Real)
Real options are specified by `rswitch(K<MODEL>, <INDEX>)`.    Diffusivity bounds are given in units of m<sup>2</sup>/s, and convective velocity bounds are given in units of m/s.
...
| Model | Index | Default | Description                                        |
| ---- | ------ | ------- | -------------------------------------------------- |
| MMM  | 1  |  0     | Separate diffusivity and convective velocity  (1: True, 0: False)                     |
|      | 2  |  0     | Convert negative diffusivity to convective velocity                       |
|      | 3  |  0     | Allow negative diffusivity output                       |
|      | 4  |  0     | Modify diffusivity using solver gradients                       |
|      | 5  |  0     | Limit large diffusivity from small gradients                       |
| | | |
...
> [!NOTE]
> - z_gelong is required to run ETGM when the Horton model is selected. <br>
> - z_etanc is only used in DBM, but is not required to run this model. <br>
